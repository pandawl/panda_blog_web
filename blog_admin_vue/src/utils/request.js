...
let BASE_REQUEST = axios.create({
  baseURL: 'http://127.0.0.1:9999/panda',
  responseType: 'json',
...
// 拦截响应
BASE_REQUEST.interceptors.response.use((config) => {
  return config
}, (error) => {
  if (error.response) {
...
const request = {
  post (url, params) {
    return BASE_REQUEST.post(url, params, {
  ...
  put (url, params) {
    ...
    return BASE_REQUEST.put(url, params, {
      transformRequest: [(params) => {
        let result = ''
        Object.keys(params).forEach((key) => {
          if (!Object.is(params[key], undefined) && !Object.is(params[key], null)) {
  ...
  get (url, params) {
    ...
    return BASE_REQUEST.get(`${url}${_params}`)
  },
  delete (url, params) {
    let _params
    if (Object.is(params, undefined)) {
    ...
    return BASE_REQUEST.delete(`${url}${_params}`)
  },
  export (url, params = {}) {

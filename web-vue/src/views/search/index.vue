  ...
  <div class="head_aricle">
    <div>
      <div class="whitebg bloglist left">
        <h2 class="htitle">搜索"<span class="result">{{search}}</span>"结果如下</h2>
        ...
        <h2 v-show="!article" style="text-align: center;">该结果下暂无文章~~~</h2>
        <!--pagelist-->
        <Pager
          v-if="!dataChanged"
          ref="pager"
...
import { getBlogESList} from "../../api/blog";
...
function stageBreakHandler(event){
 ...
 this.loading= true;
  ...
  this.search = search;
      getBlogESList(this.curPage, this.pageSize,search).then(res => {
  ...
  methods: {
    ...
    gotoPage(curPage) {
       this.loading= true;
      let search = this.$route.query.name;
      getBlogESList(curPage, this.pageSize,search).then(res => {
        this.loading= false;
        this.blogs = res.data.resultJson.list;
        this.curPage = res.data.resultJson.pageNum;
        this.total = res.data.resultJson.pages;
